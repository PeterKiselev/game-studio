 ...
 * дело с двумя решениями и дело, путь к решению которого движок не может
 * объяснить, — всё это дефекты контента, и находить их должен автор,
 * а не игрок.
 */
...
export function validateCase(source: Case): ValidationIssue[] {
  ...
  if (deduction.contradiction) {
    ...
    /*
     * Формулировка важна: решение существует и единственно, значит дело
     * разрешимо логически. Не выводится оно не «потому что требует
     * угадывания», а потому что текущим пяти правилам движка не хватает
     * средств объяснить путь. Для автора это одно и то же действие —
     * переформулировать улики, — но диагноз другой, и врать в нём нельзя.
     */
    warn(
      'not-explainable',
      `Решение единственно, но текущим правилам рассуждения не хватает, чтобы его объяснить: ` +
        `не выведено ${deduction.unresolved} клеток. Переформулируйте улики — ` +
        'иначе подсказка не доведёт игрока до конца.',
    );
  }
