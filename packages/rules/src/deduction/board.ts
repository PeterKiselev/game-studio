...
class Board {
  ...
  private applyOrderClue(clue: Clue, clueIndex: number): boolean {
    ...
    /*
     * Самоотношение: a и b — одна и та же сущность, буквально совпадающий
     * ref или уже установленная через 'same' пара. Она не может быть
     * до, после или рядом с собой — это противоречие всегда, а не иногда.
     *
     * Раньше это ловилось не прямой проверкой, а случайно: у before/after
     * асимметричное сужение кандидатов на второй итерации схлопывало
     * множество до одной позиции, и там self-сравнение падало само.
     * У adjacent отношение симметричное, сужения не происходит вообще —
     * ни одного assert() не вызывается, и клетка тихо остаётся
     * неопределённой. Полагаться на случайную сходимость нельзя:
     * проверяем самоотношение явно, а не ждём, сойдётся оно или нет.
     */
    const sameEntity =
      (clue.a.category === clue.b.category && clue.a.value === clue.b.value) ||
      this.state(clue.a, clue.b) === 'yes';

    if (sameEntity) {
      this.fail(
        `Улика ${clueIndex + 1} требует отношения «${clue.kind}» сущности с самой собой — ` +
          'это невозможно.',
      );
      return changed;
    }
